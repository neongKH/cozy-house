...
$('.our-friends__slider').slick({
	...
	slidesToScroll: 1,
	arrows: true,
	prevArrow: '<div class="our-friends__slider-arrow prev"></div>',
	nextArrow: '<div class="our-friends__slider-arrow next"></div>'
});

$('[data-toggle="nav-scroll"]').click(function (e) {
	e.preventDefault();
...
$('.menu-section').each(function () {
	...
	var thisSectionTopOffset = $(this).attr('data-section-top-offset');
	var scrollDownTopOffset = 150;
	var thisElement = $(this);

	var thisLink = $('#header_nav a[data-target="#' + thisId + '"], #header_mobile_nav a[data-target="#' + thisId + '"]');

	if (thisSectionTopOffset == '50%' && thisSectionTopOffset !== '') {
		scrollDownTopOffset = $(thisElement).outerHeight() / 2;
	}

	$(this).waypoint(function (direction) {
		if (direction === 'down') {
			$('#header_nav li, #header_mobile_nav li').removeClass('active');
			$(thisLink).parent().addClass('active');
		}
	}, {
		offset: scrollDownTopOffset
	});
	...
	$(this).waypoint(function (direction) {
		...
		offset: -$(thisElement).outerHeight() + 70
	});
});